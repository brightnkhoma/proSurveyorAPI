...
class Interpolate:
    ...
    def interpolate( self,bins : int,data : list, shapefile_path : str,contour_map : bool = True,crs : int = 4326, report :bool = False,title : str = "",model : str = "exp"):
        data = np.array(data)
        latitudes = data[:,0]
        longitudes = data[:,1]
        values = data [:,2]
        ...
        krigging_model.variogram(bins=bins)
        krigging_model.fit(model=model)
        z = []
        z.append(self.convert())
        plt.savefig("zz.png")
        
        initial_geoDataFrame = self.get_shapeFile(shape_file_path=shapefile_path)
        ...
        z.append(self.convert())
        if(report):
            report = self.createReport(title,krigging_model.evaluate(return_=True))
            plt.close()
